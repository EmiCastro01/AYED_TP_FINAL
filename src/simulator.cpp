 ...
 *
 * Simulator needs to be configurated with a netowrk and an administrator
 * object.
 *
...
void Simulator::debug(bool debug_mode) { this->debug_mode = debug_mode; }

/**
 * @brief Executes the simulation process.
 *
 */
...
/**
 * @brief Configures the simulator with the network and the administrator.
 *
 * @param network
 * @param sys_adm
 */
...
/**
 * @brief Executes a cycle of the network.
 *
 */
void Simulator::cycle() {
    ...
    cout << ">> SIMULATOR >> 4. Print Adjacency Matrix (Congestion Map)"
         << endl;
    ...
    switch (option) {
        case 0: {
            ...
            cout << "Debug Mode is used for execute cycles or steps on the "
                    "traffic"
                 << endl;
            cout << "simulator System. " << endl;
            cout << "When user press 'Choose Terminal', options for create a "
                    "new Page will be"
                 << endl;
            cout << "displayed." << endl;
            cout << "When user press 'Run a void cycle', every router, in "
                    "rising order, will be "
                 << endl;
            cout << "executed. Due to this, packets may be sent to destination "
                    "in just 1 void cycle."
                 << endl;
            cout << "When user press 'Run a void step', the user will be ask "
                    "to insert a router to execute."
                 << endl;
            cout << "'Print Congestion Map' & 'Print Connections' display maps "
                    "with the current info*."
                 << endl
                 << endl;
            cout << "* Current info may be outdated beacuse of no-updating "
                    "cycles."
                 << endl;
            cin.get();
        ...
        case 1: {
            ...
            Terminal *terminal_sender =
                this->network->get_terminal_by_id(terminal_id);
            if (terminal_sender == nullptr ||
                terminal_sender->get_type_t() == RECIEVER) {
                cout << ">> SIMULATOR >> Invalid ID or Receiver Terminal"
                     << endl;
                break;
            }
            ...
            Terminal *terminal_receiver =
                this->network->get_terminal_by_id(destination_id);
            if (terminal_receiver == nullptr ||
                terminal_receiver->get_type_t() == CLIENT) {
                cout << ">> SIMULATOR >> Invalid ID or Client Terminal" << endl;
                break;
            }
            cout << ">> SIMULATOR >> Destination terminal chosen: "
                 << destination_id << endl;
                ...
                for (int j = 0; j < this->network->get_routers_no(); j++) {
                    ...
                    if (this->network->get_adjacency_matrix()[i][j] != INFI &&
                        ...
                        cout << "Router " << i << " connected to Router " << j
                             << " with cost: "
                             << this->network->get_router_by_id(i)
                                    ->get_neighbors()
                                    .search_router(j)
                                    .cost
                             << endl;
        ...
        case 6: {
            ...
            cout << ">> SIMULATOR >> Router for Terminal " << terminal_id
                 << " is: " << terminal->get_router()->get_name() << endl;
            break;
        }
        case 7: {
            cout << ">> SIMULATOR >> Choose Terminal ID: ";
            ...
            if (terminal == nullptr || terminal->get_type_t() == CLIENT) {
                cout << ">> SIMULATOR >> Invalid Terminal ID or Client Terminal"
                     << endl;
                break;
            }
            cout << ">> SIMULATOR >> Terminal chosen: " << terminal_id << endl;
        ...
        case 8: {
            ...
            for (int i = 0; i < this->network->get_routers_no(); i++) {
                ...
                for (int j = 0; j < this->network->get_routers_no(); j++) {
                    ...
                    if (this->network->get_adjacency_matrix()[i][j] != INFI &&
                        i != j) {
                        cout << this->network->get_router_by_id(i)
                                    ->get_neighbors()
                                    .search_router(j)
                                    .cost
                             << " ";
    ...
    if (this->cycle_counter % 2 == 0) this->sys_adm->calculate_optimal_routes();
    this->cycle_counter++;
}